...
export default function RequestConfirmCancellationPage() {
  const { id } = useParams();
  const [state, setState] = useState<ActionState>('loading');
  const [message, setMessage] = useState('Confirmation de l\'annulation en cours...');
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        await confirmCancellation(Number(id));
        setState('success');
        setMessage('Annulation confirmee avec succes.');
      } catch (error: any) {
        setState('error');
        setMessage(
          error?.message || error?.error?.message || 'Impossible de confirmer l\'annulation.'
        );
      }
    };
