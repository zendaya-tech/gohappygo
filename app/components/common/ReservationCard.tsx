            ...
            <div className="flex-1 p-4">
                ...
                {reservation.status === 'waiting_proposal' && (
                    <div className="mt-4 flex items-center gap-3">
                        <button onClick={() => onApprove?.(reservation.id)} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm hover">Approve</button>
                        <button onClick={() => onReject?.(reservation.id)} className="px-4 py-2 rounded-lg border border-red-300 text-red-600 text-sm hover">Reject</button>
                    </div>
                )}
            </div>

            <div className="w-64 p-4 border-l border-gray-200 hidden md:block">
                <div className="text-sm font-semibold text-gray-900 mb-2">Customer Details</div>
                ...
                <button onClick={() => onContact?.(reservation.id)} className="mt-4 w-full px-4 py-2 rounded-lg border text-sm hover">Contact</button>
            </div>
        </div>
    );
}
