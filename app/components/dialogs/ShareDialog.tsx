...
export default function ShareDialog({ open, onClose, listing }: ShareDialogProps) {
  ...
  const shareOptions = [
      ...
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            ...
            strokeWidth={2}
            d="M8 16h8M8 12h8m-9 8h10a2 2 0 002-2V6a2 2 0 00-2-2H7a2 2 0 00-2 2v8"
          />
        </svg>
      ),
    ...
    },
    {
      ...
      action: () =>
        window.open(
          `mailto:?subject=${encodeURIComponent(listing.title)}&body=${encodeURIComponent(window.location.href)}`
        ),
    },
    {
      name: 'Messages',
    ...
    },
    {
      ...
      action: () =>
        window.open(
          `https://api.whatsapp.com/send?text=${encodeURIComponent(`${listing.title} - ${window.location.href}`)}`
        ),
    },
    {
      name: 'Messenger',
      ...
      action: () =>
        window.open(
          `https://www.facebook.com/dialog/send?link=${encodeURIComponent(window.location.href)}&app_id=YOUR_APP_ID`
        ),
    },
    {
      name: 'Facebook',
