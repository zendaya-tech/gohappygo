...
export default function ChatWidget() {
      ...
      const build = (name: string, offsetMin: number): Message[] => [
        {
          id: 'm1',
          sender: 'them',
          text: `Salut, c'est ${name}.`,
          sentAt: new Date(now - offsetMin * 60_000 - 120000).toISOString(),
        },
        {
          id: 'm2',
          sender: 'me',
          text: 'Hello! Dis-moi en plus.',
          sentAt: new Date(now - offsetMin * 60_000 - 60000).toISOString(),
        },
        {
          id: 'm3',
          sender: 'them',
          text: "Parfait, on s'organise ici.",
          sentAt: new Date(now - offsetMin * 60_000).toISOString(),
        },
      ];
      ...
      >
        <svg className="mx-auto h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor">
          ...
          <path
            ...
            d="M21 12c0 3.866-3.582 7-8 7-1.07 0-2.086-.17-3.01-.48L4 20l1.49-3.735C5.177 15.34 5 14.69 5 14c0-3.866 3.582-7 8-7s8 3.134 8 7z"
          />
        </svg>
      </button>
    );
  ...
  return (
    <div className="fixed bottom-5 right-5 z-40 w-[min(100vw-20px,1000px)] h-[70vh] rounded-2xl border border-gray-200 bg-white shadow-2xl grid grid-cols-1 md:grid-cols-[320px_1fr] overflow-hidden ">
      ...
      <button
        type="button"
        aria-label={t('common.chat.closeChat')}
      ...
      <aside className="flex h-full flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
          <h3 className="text-sm font-semibold text-gray-900">{t('common.chat.title')}</h3>
          <span className="text-xs text-gray-500">{conversations.length}</span>
        </div>
        <div className="p-2 border-b border-gray-200">
          <input
            placeholder={t('common.chat.search')}
        ...
        <div className="border-t border-gray-200 p-3">
          <button
            type="button"
            className="w-full rounded-lg bg-indigo-600 px-3 py-2 text-sm font-semibold text-white hover"
          >
            {t('common.chat.newConversation')}
          </button>
        </div>
      </aside>
      ...
      <section className="flex h-full flex-col">
        {selectedId ? (
          ...
          <div className="m-auto text-center text-gray-500 text-sm">
            {t('common.chat.selectConversation')}
          </div>
        )}
      </section>
    </div>
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
        ...
        <button className="text-xs text-gray-500 hover">{t('common.chat.info')}</button>
      </div>
      ...
      <div id="chat-scroll-container" className="flex-1 overflow-y-auto bg-gray-50 px-3 py-4">
        <div className="space-y-2">
          {messages.map((m) => (
            ...
            >
              <div
                className={`max-w-[75%] rounded-2xl px-3 py-2 text-sm shadow ${m.sender === 'me' ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-white text-gray-900 rounded-bl-sm border border-gray-200'}`}
              >
                <p className="whitespace-pre-wrap break-words">{m.text}</p>
                <div
                  className={`mt-1 text-[10px] ${m.sender === 'me' ? 'text-indigo-100' : 'text-gray-400'}`}
                >
                  {formatDate(m.sentAt)}
                </div>
              </div>
      ...
      <div className="border-t border-gray-200 p-3">
        ...
        >
          <textarea
            ...
            rows={1}
            placeholder={t('common.chat.writeMessage')}
